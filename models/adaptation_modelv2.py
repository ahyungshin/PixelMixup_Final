...
class CustomModel():
    def __init__(self, opt, logger, isTrain=True):
        ...
        self.PixelMixup_blocks_Q = nn.Sequential(
            nn.Conv2d(in_channels=19, out_channels=19, kernel_size=5, stride=3),
            nn.BatchNorm2d(num_features=19),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=19, out_channels=19, kernel_size=5, stride=3),
            nn.BatchNorm2d(num_features=19),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=19, out_channels=19, kernel_size=1, stride=1),
            nn.BatchNorm2d(num_features=19),
            nn.ReLU(inplace=True),)

        self.PixelMixup_blocks_K = nn.Sequential(
            nn.Conv2d(in_channels=19, out_channels=19, kernel_size=5, stride=3),
            nn.BatchNorm2d(num_features=19),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=19, out_channels=19, kernel_size=5, stride=3),
        ...
        self.PixelMixup_blocks_V = nn.Sequential(
            nn.Conv2d(in_channels=19, out_channels=19, kernel_size=5, stride=3),
            nn.BatchNorm2d(num_features=19),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=19, out_channels=19, kernel_size=5, stride=3),
        ...
        self.PixelMixup_blocks_Q_DP = self.init_device(self.PixelMixup_blocks_Q, gpu_id=self.default_gpu, whether_DP=True)
        self.PixelMixup_blocks_K_DP = self.init_device(self.PixelMixup_blocks_K, gpu_id=self.default_gpu, whether_DP=True)
        self.PixelMixup_blocks_V_DP = self.init_device(self.PixelMixup_blocks_V, gpu_id=self.default_gpu, whether_DP=True)
        # self.PixelMixup_linear = nn
        ...
        self.PixelMixup_blocks_Q_DP.apply(init_weights)
        self.PixelMixup_blocks_K_DP.apply(init_weights)
        self.PixelMixup_blocks_V_DP.apply(init_weights)


        self.PixelMixup_Opti = optimizer_cls([{'params':self.PixelMixup_blocks_Q_DP.parameters(), 'lr':optimizer_params['lr']}, 
                                        {'params':self.PixelMixup_blocks_K_DP.parameters(), 'lr':optimizer_params['lr']},
                                        {'params':self.PixelMixup_blocks_V_DP.parameters(), 'lr':optimizer_params['lr']}], **optimizer_params)
        self.optimizers.extend([self.PixelMixup_Opti])

        self.PixelMixup_Schedule = get_scheduler(self.PixelMixup_Opti, opt)
        self.schedulers.extend([self.PixelMixup_Schedule])

        self.cross_entropy_criterion = nn.CrossEntropyLoss(ignore_index=250, reduction='none')
    ...
    def step(self, iter, source_x, source_label, target_x, queue, queue_label, queue_out, use_the_queue, use_the_queue_out, target_imageS=None, target_params=None, target_lp=None, 
        ...
        target_out_ = target_out['out'].clone()
        ...
        with torch.no_grad():
            ...
            if pixelmixup is True:
                ...
                ent = F.interpolate(ent.to(dtype=torch.float32), scale_factor=4)
        ...
        loss = loss + loss_CTS + self.opt.SL_lambda*loss_SL + self.opt.contrast_lambda*loss_contrast + loss_pixelmix #!!!!!!!!!!!!!!
        loss.backward()
