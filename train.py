...
def train(opt, logger):
    ...
    if opt.stage == 'stage1':
        objective_vectors = torch.load(os.path.join('pretrained_models', 'prototypes_on_{}_from_{}'.format(opt.tgt_dataset, opt.model_name)))
        model.objective_vectors = torch.Tensor(objective_vectors).to(0)
        class_distribution = np.load(os.path.join('Pseudo', 'class_distribution.npy'))
        model.class_distribution = torch.Tensor(class_distribution).to(0)
        # init prototype layer
        with torch.no_grad():
            w = torch.nn.functional.normalize(model.objective_vectors, dim=1, p=2)
            model.prototypes.data = w
            model.prototypes_ema.data = w
        queue = torch.zeros(opt.queue_length, 256).to(device)
        queue_label = torch.zeros(opt.queue_length, 1).to(device)
        queue_out = torch.zeros(opt.queue_length, opt.n_class).to(device)
        use_the_queue = False
        use_the_queue_out = False
    ...
    for epoch in range(start_epoch, opt.epochs):
        for data_i in datasets.target_train_loader:
            ...
            if opt.stage == 'warm_up':
                ...
                loss, loss_CTS, loss_consist, loss_SL, loss_contrast, queue, queue_out = model.step(i, images, labels, target_image, queue, queue_label, queue_out, use_the_queue, use_the_queue_out, target_imageS, target_params, target_lp, target_lpsoft, target_image_full, target_weak_params)
            else:
            ...
            if (i + 1) % opt.print_interval == 0:
                if opt.stage == 'warm_up':
                    ...
                    fmt_str = "Epochs [{:d}/{:d}] Iter [{:d}/{:d}]  loss: {:.4f}  loss_CTS: {:.4f}  loss_consist: {:.4f}  loss_SL: {:.4f}  loss_contrast: {:.4f} Time/Image: {:.4f}"
                    print_str = fmt_str.format(epoch+1, opt.epochs, i + 1, opt.train_iters, loss, loss_CTS, loss_consist, loss_SL, loss_contrast, time_meter.avg / opt.bs)
                else:
...
def validation(model, logger, datasets, device, running_metrics_val, iters, opt=None):
    ...
    for net in model.nets:
        ...
        new_state = {
            ...
            "prototypes_ema": model.prototypes_ema,
        }
